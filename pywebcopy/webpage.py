...
from .compat import basestring as string_types
...
class BaseWebPage(BaseIncrementalParser):
    ...
    def set_source(self, source, encoding=None, base_url=None):
        ...
        Use a file like object as source which has a :meth:`.read` or
        you can put in file path if you like.

        :param source: file_like_object or file path
        :param encoding: source encoding
        :param base_url: url which would be used to resolve relative paths
        """
        if not hasattr(source, 'read') and len(source) < 256:
            try:
        ...
        if base_url:
            assert isinstance(base_url, string_types), "Url must be a string!"
            self._url = base_url
            self._url_obj = None
        self._source = source
    ...
    def save_html(self, file_name=None, raw_html=False):
        ...
        if raw_html:
            ...
            if self.root is None:
                self.__parse__()
            # if still no root tree
            if not hasattr(self.root, 'getroottree'):
                raise ParseError("Tree is not being generated by parser!")
            self.root.getroottree().write(file_name, method="html")
            LOGGER.success("WebPage saved successfully to %s" % self.utx.file_path)
...
class WebPage(BaseWebPage):
    ...
    def get(self, url, **params):
        ...
        req = SESSION.get(url, stream=True, **params)
