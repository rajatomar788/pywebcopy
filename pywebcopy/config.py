...
import os
import pywebcopy
from pywebcopy.utils import hostname, urljoin
from pywebcopy import VERSION, LOGGER, _n_fileLogger
from pywebcopy.structures import CaseInsensitiveDict, RobotsTxt
...
default_config = {
    ...
    'allowed_file_ext': ['.html', '.php', '.asp', '.aspx', '.htm', '.xhtml', '.css',
                         ...
                         '.jpg', '.png', '.ttf', '.eot', '.otf', '.woff', '.woff2', '.download',],
    'parser': 'lxml',
    'http_headers': {
...
class ConfigHandler(CaseInsensitiveDict):
    ...
    def setup_paths(self, project_folder, project_name):
        ...
        LOGGER.addHandler(_n_fileLogger(self['log_file'], 'w'))
    ...
    def setup_config(self, project_url, project_folder, project_name, **kwargs):
        ...
        pywebcopy.SESSION.headers.update(self['http_headers'])

        # Default base configuration is done right away so it at least sets base files and folder
        self.setup_paths(project_folder, project_name or hostname(project_url))
